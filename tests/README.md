...
Core tests take longer than any other Ombre tests, due to the high amount of computational work involved in validating core components.
...
To run only Ombre's core tests (after building):
...
To run only Ombre's crypto tests (after building):
...
To run only Ombre's hash tests (after building):
...
To run only Ombre's performance tests (after building):
...
To run only Ombre's unit tests (after building):
